...
//
// NOTE: pdf-parse, mammoth, and tesseract are all lazy-required inside the
// functions that need them. pdf-parse in particular has a bug where the bare
// `require('pdf-parse')` can run a demo that reads a non-existent test file
// and crash the process on boot. Loading it on first use avoids that.
...
async function extractPdf(buf) {
  // Pull from the internal path to skip pdf-parse's top-level demo block.
